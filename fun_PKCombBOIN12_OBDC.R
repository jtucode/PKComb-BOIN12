...
fun_PKCombBOIN12_OBDC <- function(doseDT, J, K, pT, u11, u00, zeta1) {
  ...
  for (jj in 1:J) for (kk in 1:K) {
    ...
    if (keep_mat[jj, kk] == 0) ubar[jj, kk] <- -100
    if (n_mat[jj, kk] == 0) ubar[jj, kk] <- -100
  }
  
  if (max(ubar) <= -100) return(c(99, 99))
